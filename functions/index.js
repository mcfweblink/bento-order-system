...
exports.getPublicData = onCall({ enforceAppCheck: true }, async (request) => {
  try {
    ...
    const products = productsSnapshot.docs.map((doc) => {
      ...
      // priceが数値でない場合は0をデフォルトにする
      const price = typeof productData.price === 'number' && productData.price >= 0 ? productData.price : 0;

      // discountPriceが数値でない場合は、通常価格(price)をフォールバックとして使用する
      ...
      return {
        id: doc.id,
        ...productData, // 元のデータを展開
        price: price, // 安全な値で上書き
        discountPrice: discountPrice // フォールバック処理後の値で上書き
      };
    });
    // ★★★★★ ここまでが修正箇所 ★★★★★
